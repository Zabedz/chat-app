...
export const resolvers = {
    ...
    Mutation: {
        createMessage: (
            _parent: unknown,
            {content, userId, chatRoomId}: { content: string; userId: string; chatRoomId: string }) => {
            const message: Message = {id: (messages.length + 1).toString(), content, userId, chatRoomId};
            messages.push(message);
            pubsub.publish(MESSAGE_SENT, {messageSent: message});
            return message;
        },
        createChatRoom: (_parent: unknown, {name}: { name: string }) => {
            const chatRoom: ChatRoom = {id: (chatRooms.length + 1).toString(), name};
    ...
    ChatRoom: {
        messages: (chatRoom: ChatRoom) => messages.filter((m) => m.chatRoomId === chatRoom.id),
    },
};
